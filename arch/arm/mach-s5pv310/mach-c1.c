...
static int m5mo_power_on(void)
{
	...
	// enable ldo12
	regulator = regulator_get(NULL, "cam_sensor_core");
	if (IS_ERR(regulator))
		return -ENODEV;
	...
	gpio_direction_output(GPIO_8M_AF_EN, 1);
...
static int m5mo_power_down(void)
{
	...
	gpio_direction_output(GPIO_8M_AF_EN, 0);
	gpio_free(GPIO_8M_AF_EN);
#else
	regulator = regulator_get(NULL, "cam_af");
	if (IS_ERR(regulator))
...
void s3c_csis_power(int enable)
{
	...
	dev = fimc_get_active_device();
	if (!dev) {
		printk(KERN_ERR "%s: ERROR: failed to get dev\n", __func__);
		return -ENODEV;
	}

	/* printk("%s(dev name = %s)  in \n", __func__, dev_name(dev)); */

	if (enable) {
		regulator = regulator_get(dev, "vmipi_1.1v");
		if (IS_ERR(regulator))
			goto error_out;
		regulator_enable(regulator);
		regulator_put(regulator);
...
error_out:
	printk(KERN_ERR "%s: ERROR: failed to power mipi.\n", __func__);
	return -ENODEV;
}
...
#if defined (CONFIG_MACH_C1_REV01) || defined(CONFIG_MACH_C1_REV02)
...
#else
	.sda_pin                = CODEC_VT_SDA_18V,
	.scl_pin                = CODEC_VT_SCL_18V,
#endif
        .udelay                 = 5,    /* 250KHz */
        .sda_is_open_drain      = 0,
        .scl_is_open_drain      = 0,
...
struct gpio_keys_button c1_buttons[] = {
	...
	{ .code=102, .gpio=GPIO_OK_KEY,   .active_low=1, }, /* ok key */
};
...
static unsigned int c1_sleep_gpio_table[][3] = {
	...
	{ S5PV310_GPA0(3), S3C_GPIO_SLP_PREV,	S3C_GPIO_PULL_NONE},
	{ S5PV310_GPA0(4), S3C_GPIO_SLP_INPUT,	S3C_GPIO_PULL_DOWN},
	{ S5PV310_GPA0(5), S3C_GPIO_SLP_OUT0,	S3C_GPIO_PULL_NONE},
	{ S5PV310_GPA0(6), S3C_GPIO_SLP_INPUT,	S3C_GPIO_PULL_DOWN},
	{ S5PV310_GPA0(7), S3C_GPIO_SLP_OUT0,	S3C_GPIO_PULL_NONE},
	...
	{ S5PV310_GPB(2),  S3C_GPIO_SLP_OUT1,	S3C_GPIO_PULL_NONE},
	{ S5PV310_GPB(3),  S3C_GPIO_SLP_OUT0,	S3C_GPIO_PULL_NONE},
	{ S5PV310_GPB(4),  S3C_GPIO_SLP_PREV,	S3C_GPIO_PULL_NONE},
	{ S5PV310_GPB(5),  S3C_GPIO_SLP_OUT1,	S3C_GPIO_PULL_NONE},
	{ S5PV310_GPB(6),  S3C_GPIO_SLP_OUT0,	S3C_GPIO_PULL_NONE},
	{ S5PV310_GPB(7),  S3C_GPIO_SLP_PREV,	S3C_GPIO_PULL_NONE},
	...
	{ S5PV310_GPK1(0),  S3C_GPIO_SLP_INPUT,	S3C_GPIO_PULL_DOWN},
	{ S5PV310_GPK1(1),  S3C_GPIO_SLP_INPUT,	S3C_GPIO_PULL_DOWN},
...
{
        printk(KERN_DEBUG "smdkc210_power_off.\n");
	...
	*/
        ...
        printk(KERN_DEBUG "Should not reach here!\n");
}
...
#endif
	printk("spi0_dev=0x%x, spi1_dev=0x%x, spi2_dev=0x%x\n", spi0_dev, spi1_dev, spi2_dev);
...
static void __init s5pv310_reserve(void)
{
	...
	static const char map[] __initconst =
		...
		"s3c-fimc.3=fimc3;s5p-tvout=tvout;s3c-mfc=mfc,mfc0,mfc1;s5p-rp=srp;"
		"s5p-jpeg=jpeg;"
		"s5p-fimg2d=fimg2d";
	int i;
